...
%----------------------------------------------------------------------------------------
\begin{frame}{Real soil behavior}
	\mode<beamer>{
	\begin{figure}
		\includegraphics[width=\textwidth]{figs/real-soil.png}
	\end{figure}
	}
\end{frame}
...
\begin{enumerate}
	\item $(\sigma_1 - \sigma_3)_{max}$ increases with increasing $\sigma_3$.
	\item $E_t$ increases with increasing $\sigma_3$.
	\item $E_t$ decreases with increasing $\varepsilon_a$.
	\item $\varepsilon_v$ increasses with increasing $\sigma_3$.
\end{enumerate}
Could also consider: soil softening, dilation, rate effects...
Duncan model captures 1 - 4.
}
%----------------------------------------------------------------------------------------
\begin{frame}
...
\note{
$R_f$ is a curve fitting parameter to control $\varepsilon_f$.

$R_f = \frac{(\sigma_1 -  \sigma_3)_{f}}{(\sigma_1 -  \sigma_3)_{ULT}}$

As $R_f \rightarrow 1$ curve is more hyperbolic, more ductile and $\varepsilon_f$ increases. 

Lower $R_f$, more brittle. $\varepsilon_f$ decreases. 

For most soils, $R_f = 0.6 - 0.95$ with a first guess of 0.8.
}

%----------------------------------------------------------------------------------------
\begin{frame}{Drawbacks of hyperbolic model}
	\textbf{Positive aspects}
	\begin{itemize}
		\item Relatively simple and easy to use
		\item Well validated \& calibrated
		\item Based solely on TX testing
		\item Good for loading conditions (dams)
	\end{itemize}
	\textbf{Original short comings}
	\begin{itemize}
		\item Not an effective stress model (no treatment of $\Delta u$) - Plaxis can do
		\item No strain softening (still a problem)
		\item No dilation - important for dense sand / OC clay (Plaxis can do)
		\item Empirical model
		\item Poor performance in excavation problems (where it is linear elastic completely) 
	\end{itemize}
\end{frame}
\note{
- exceptions: excavations where base-heave is the mode of failure (loading of soil at base).
}
